...
>
> This is the chronological development log kept while ION COMMAND was built.
> It records what was done on which day, including measurements that were later
> retracted and configurations that have since changed. **Do not read it as a
> description of the current system.**
>
> For the current state see [../IMPLEMENTATION_STATUS.md](../IMPLEMENTATION_STATUS.md),
> [../COMPONENTS.md](../COMPONENTS.md) and [../../CHANGELOG.md](../../CHANGELOG.md).

# Implementation status

Status date: **2026-07-19** (fifteenth pass: marker pictograms and
Europe-wide ADS-B coverage).

## Marker pictograms and ADS-B coverage (2026-07-19)

- **Pictogram markers**: point markers are now camera-facing quads showing a
  per-type glyph from a generated 4x2 icon atlas (dot, antenna, aircraft,
  satellite, lightning bolt, ionosonde, seismogram ring, spare). Domains
  declare their glyph via the generic `visual.icon` envelope property
  (`signal`, `aircraft`, `satellite`, `lightning`, `sounding`,
  `earthquake`); the renderer only knows atlas tiles plus a replay-era
  fallback keyed by domain name. Each icon type has its own tint (hamradio
  cyan, aviation amber, satellites ice white, lightning violet, sounders
  green, quakes red).
- **Rendering**: single instanced quad pool (`M_MarkerIcon`), billboarding
  done in the material's world position offset. Per-instance custom data
  contract: 0 = atlas tile, 1-3 = RGB tint, 4-6 = marker world origin — the
  origin is passed as data so the billboard math never depends on the
  vertex factory's Local->World including the instance transform. Atlas is
  generated by `generate_visual_sources.py` (white-on-black silhouettes,
  4x supersampled), imported linear (sRGB off).
- **Aviation "centered on home" report**: verified against 19'514 live
  observations — max distance was exactly 370 km = 200 nm, i.e. the point
  query radius around the configured QTH, not a projection bug. Coverage
  widened instead: `live.json` now runs five `aviation.adsb` instances
  (QTH 200 nm plus London/Madrid/Rome/Berlin at 250 nm, 30 s polls);
  markers dedupe by hex-based entity id where circles overlap.
