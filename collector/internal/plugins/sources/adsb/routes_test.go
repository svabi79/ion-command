...
func TestResolverCachesPositiveAndNegativeAnswers(t *testing.T) {
	...
	resolver.fetch = func(_ context.Context, callsign string) ([]byte, int, error) {
		fetches.Add(1)
		if callsign == "AFR1084" {
			return []byte(routeResponseLive), http.StatusOK, nil
		}
		return []byte(`{"response":"unknown callsign"}`), http.StatusNotFound, nil
	}
