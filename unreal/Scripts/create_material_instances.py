...
def build_marker_icon_master(icon_texture) -> unreal.Material:
    ...
    right_default = expression(material, unreal.MaterialExpressionNormalize, -700, 520)
    MEL.connect_material_expressions(right_raw, "", right_default, "")
    up_default = expression(material, unreal.MaterialExpressionCrossProduct, -700, 600)
    ...
    # The heading projected into the billboard plane becomes the icon's up
    # axis (glyphs point "up" toward their course); zero heading keeps the
    # world-up default. Blend by presence mask instead of an If node (the If
    # pin names silently fail to connect from python).
    heading_x = expression(material, unreal.MaterialExpressionPerInstanceCustomData, -1500, 660)
    heading_x.set_editor_property("data_index", 7)
    heading_y = expression(material, unreal.MaterialExpressionPerInstanceCustomData, -1500, 740)
    heading_y.set_editor_property("data_index", 8)
    heading_z = expression(material, unreal.MaterialExpressionPerInstanceCustomData, -1500, 820)
    heading_z.set_editor_property("data_index", 9)
    heading_xy = expression(material, unreal.MaterialExpressionAppendVector, -1340, 700)
    MEL.connect_material_expressions(heading_x, "", heading_xy, "A")
    MEL.connect_material_expressions(heading_y, "", heading_xy, "B")
    heading = expression(material, unreal.MaterialExpressionAppendVector, -1220, 720)
    MEL.connect_material_expressions(heading_xy, "", heading, "A")
    MEL.connect_material_expressions(heading_z, "", heading, "B")

    heading_dot_fwd = expression(material, unreal.MaterialExpressionDotProduct, -1060, 700)
    MEL.connect_material_expressions(heading, "", heading_dot_fwd, "A")
    MEL.connect_material_expressions(forward, "", heading_dot_fwd, "B")
    fwd_component = expression(material, unreal.MaterialExpressionMultiply, -940, 720)
    MEL.connect_material_expressions(forward, "", fwd_component, "A")
    MEL.connect_material_expressions(heading_dot_fwd, "", fwd_component, "B")
    heading_planar = expression(material, unreal.MaterialExpressionSubtract, -820, 700)
    MEL.connect_material_expressions(heading, "", heading_planar, "A")
    MEL.connect_material_expressions(fwd_component, "", heading_planar, "B")
    up_heading = expression(material, unreal.MaterialExpressionNormalize, -700, 700)
    MEL.connect_material_expressions(heading_planar, "", up_heading, "")
    right_heading = expression(material, unreal.MaterialExpressionCrossProduct, -700, 780)
    MEL.connect_material_expressions(up_heading, "", right_heading, "A")
    MEL.connect_material_expressions(forward, "", right_heading, "B")

    # presence = saturate(dot(planarHeading, planarHeading) * 1e6): zero for
    # static markers AND for the degenerate camera-aligned heading.
    heading_len = expression(material, unreal.MaterialExpressionDotProduct, -820, 860)
    MEL.connect_material_expressions(heading_planar, "", heading_len, "A")
    MEL.connect_material_expressions(heading_planar, "", heading_len, "B")
    huge = expression(material, unreal.MaterialExpressionConstant, -820, 940)
    huge.set_editor_property("r", 1000000.0)
    presence_raw = expression(material, unreal.MaterialExpressionMultiply, -700, 880)
    MEL.connect_material_expressions(heading_len, "", presence_raw, "A")
    MEL.connect_material_expressions(huge, "", presence_raw, "B")
    presence = expression(material, unreal.MaterialExpressionSaturate, -600, 880)
    MEL.connect_material_expressions(presence_raw, "", presence, "")

    right = expression(material, unreal.MaterialExpressionLinearInterpolate, -560, 560)
    MEL.connect_material_expressions(right_default, "", right, "A")
    MEL.connect_material_expressions(right_heading, "", right, "B")
    MEL.connect_material_expressions(presence, "", right, "Alpha")
    up = expression(material, unreal.MaterialExpressionLinearInterpolate, -560, 640)
    MEL.connect_material_expressions(up_default, "", up, "A")
    MEL.connect_material_expressions(up_heading, "", up, "B")
    MEL.connect_material_expressions(presence, "", up, "Alpha")
